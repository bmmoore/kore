...
instance ProofSystem
  ...
  where
    ...
    checkDerivation conclusion (Forall variable phi1 phi2) =
    ...
    checkDerivation conclusion (Necessitation symbol idx phi) =
        checkNecessitation symbol idx phi conclusion
...
checkNecessitation
    ...
    -> CommonMetaPattern
    -> Either (Error MLError) ()
checkNecessitation symbol idx hypothesis conclusion
  = do
    (sym, patterns) <-
        case conclusion of
            Fix
            ...
            _ -> koreFail
                    ("Expected a meta-pattern of the form " ++ kind1 ++ ".")
    testFormulaEquality
        sym (nameInKind1 "#sigma{...}")
        symbol (nameInProposition "symbol")
    koreFailWhen (idx < 0)
        (nameInProposition "idx" ++ " must not be negative.")
    koreFailWhen (idx >= length patterns)
        (  nameInProposition "idx"
        ++ " must be lower than the argument count for symbol ("
        ++ show (length patterns)
        ++ ")."
        )
    let negation = patterns !! idx
    psi <-
        case negation of
            Fix ( NotPattern Not { notChild = a } ) -> return a
            _ -> koreFail
                ("Expected a meta-pattern of the form " ++ kind2 ++ ".")
    testFormulaEquality
        psi (nameInKind2 "psi")
        hypothesis (nameInProposition "phi")
  where
    kind1 = "(#not (#sigma1{...}(...)))"
    kind2 = "(#not (#sigma1{... (not psi) ...}(...)))"
    nameInKind1 name = name ++ " in " ++ kind1
    nameInKind2 name = name ++ " in " ++ kind2
    nameInProposition name =
        name ++ " in Necessitation(symbol, idx, phiId)"
