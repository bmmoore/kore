...
-- form phi, deduce (not sigma(..., not phi, ...))
necessitationTests :: TestTree
necessitationTests =
    testGroup "Necessitation tests"
        [ runTestScript "Fails if indexes do not match"
            [ testAddGoal phi (NewGoalId 1)
            , testAddGoal
                (metaNot phiSort
                    (metaSigma phiSort psi (metaNot phiSort phi) psi)
                )
                (NewGoalId 2)
            , expectFailure
                (testNecessitation
        ...
        , runTestScript "Fails if symbols do not match"
            ...
            , testAddGoal
                (metaNot phiSort
                    (metaSigmoid phiSort psi (metaNot phiSort phi) psi)
                )
                (NewGoalId 2)
            , expectFailure
                (testNecessitation
        ...
        , runTestScript "Fails if formulas do not match"
            [ testAddGoal phi1 (NewGoalId 1)
            , testAddGoal
                (metaNot phiSort
                    (metaSigma phiSort psi (metaNot phiSort phi) psi)
...
necessitation
    ...
    -> GoalId
    -> MLProof -> Either String MLProof
necessitation symbol idx hypothesisId conclusionId proof =
    stringError propagateNecessitation'
  where
    propagateNecessitation' = do
        conclusionFormula <- lookupFormula conclusionId proof
        derive
            proof
            conclusionId
            conclusionFormula
            (Necessitation symbol idx hypothesisId)
...
testNecessitation
    ...
    -> GoalId
    -> (String, MLProof -> Either String MLProof)
testNecessitation symbol idx hypothesisId conclusionId =
    ( "necessitation with sigma=" ++ show symbol
        ++ " idx=" ++ show idx
        ++ " hypothesis=" ++ show hypothesisId
    , necessitation symbol idx hypothesisId conclusionId
    )
